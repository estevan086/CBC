...
sap.ui.define([
	...
	return Controller.extend("cbc.co.simulador_costos.controller.DataDefault.Icoterm.GridIcoterm", {
		...
		initSampleDataModel : function() {
			...
			jQuery.ajax("model/Icoterm.json", {
				dataType: "json",
				success: function(oData) {
					...
					var aSuppliersData = [];
					var aCategoryData = [];
					for (var i = 0; i < oData.ICOTERM.length; i++) {
						var oProduct = oData.ICOTERM[i];
						if (oProduct.ICOTERM && jQuery.inArray(oProduct.ICOTERM, aTemp1) < 0) {
							aTemp1.push(oProduct.ICOTERM);
							aSuppliersData.push({Name: oProduct.ICOTERM});
						}
						if (oProduct.ID && jQuery.inArray(oProduct.ID, aTemp2) < 0) {
							aTemp2.push(oProduct.ID);
							aCategoryData.push({Name: oProduct.ID});
						}
