...
sap.ui.define([
	...
	return Controller.extend("cbc.co.simulador_costos.controller.DataDefault.Version.GridVersion", {
		...
		switchState : function(sKey) {
			var oTable = this.byId("tblVersion");
			var iCount = 0;
		...
		frmLogisticPress: function(oEvent) {
			this.LogisticaDisplay = sap.ui.xmlfragment("cbc.co.simulador_costos.view.Utilities.fragments.AdminVersionDisplay", this);
		...
		getSelectedIndices: function(evt) {
			var aIndices = this.byId("tblVersion").getSelectedIndices();
			var sMsg;
		...
		sortCategories : function(oEvent) {
			...
			var oTable = oView.byId("tblVersion");
		...
		_filter : function() {
			...
			this.byId("tblVersion").getBinding("rows").filter(oFilter, "Application");
		},
